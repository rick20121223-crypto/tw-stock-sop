...
個股/ETF，整份清單（17檔）一起跑會太慢，所以維持「點進單一股票才查」。
...
with st.spinner("正在依序分析清單內所有股票的週+日整合結論（首次載入較久，之後15分鐘內會用快取）..."):
    rows, errors = [], []
    for name, code, market in unique_watchlist():
        result = analyze_stock(name, code, market, api_token, start_date)
